...
func buildFields() []SettingField {
	...
	add(
		...
		boolField("triage.style_learning", "STYLE_LEARNING", "triage", "Подстройка под мой стиль",
			"Черновики ответов учитывают ваши прошлые ответы: примеры из этого же чата и профиль стиля, который обновляется раз в неделю", "true",
			func(s *domain.Settings) *bool { return &s.StyleLearning }),
		intField("triage.style_examples", "STYLE_EXAMPLES", "triage", "Примеров моих ответов",
			"Сколько ваших последних ответов из этого чата видит модель (чем больше, тем точнее стиль, но длиннее запрос)", "15", 0, 50,
			func(s *domain.Settings) *int { return &s.StyleExamples }),
