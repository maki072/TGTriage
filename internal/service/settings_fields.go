...
func buildFields() []SettingField {
	...
	add(
		...
		stringField("helpdesk.about", "HELPDESK_ABOUT", "helpdesk", "О сервисе поддержки",
			"Чем занимается поддержка — помогает модели отличать обращения от шума", "", KindText, maxLen(2000),
	...
	add(
		...
		boolField("triage.paused", "", "triage", "⏸ Пауза триажа", "Сообщения сохраняются, но не анализируются", "false",
