...
func TestHelpdeskRepo(t *testing.T) {
	...
	got, err := r.UserByTopic(ctx, -1001, 55)
	if err != nil || got.UserID != 10 || got.Source != "site" || got.AwaitingSince == nil {
		t.Fatalf("user by topic: %+v err=%v", got, err)
	}
	if _, err := r.UserByTopic(ctx, -1001, 999); err != domain.ErrNotFound {
		t.Errorf("unknown topic: expected ErrNotFound, got %v", err)
	}

	list, total, err := r.ListUsers(ctx, domain.HelpdeskUserFilter{Limit: 10})
	if err != nil || total != 2 || list[0].UserID != 10 {
		t.Fatalf("waiting users must come first: total=%d %+v err=%v", total, list, err)
	}
	if _, n, _ := r.ListUsers(ctx, domain.HelpdeskUserFilter{AwaitingOnly: true}); n != 1 {
		t.Errorf("awaiting filter: %d", n)
	}
	if _, n, _ := r.ListUsers(ctx, domain.HelpdeskUserFilter{Query: "@ivan"}); n != 0 {
		t.Errorf("query by @username must not match LIKE with @: %d", n)
	}
	if _, n, _ := r.ListUsers(ctx, domain.HelpdeskUserFilter{Query: "Пётр"}); n != 1 {
		t.Errorf("query by name: %d", n)
	}

	due, err := r.DueReminders(ctx, time.Now().Add(-15*time.Minute))
	...
	if due, _ := r.DueReminders(ctx, time.Now().Add(-15*time.Minute)); len(due) != 0 {
	...
	if m, err := r.MessageByUserMsg(ctx, 10, 5); err != nil || m.GroupMsgID != 900 {
		t.Errorf("by user msg: %+v err=%v", m, err)
	}
	if m, err := r.MessageByGroupMsg(ctx, -1001, 901); err != nil || m.Direction != domain.HelpdeskOut || m.UserMsgID != 6 {
		t.Errorf("by group msg: %+v err=%v", m, err)
	}
	if around, _ := r.MessagesAround(ctx, -1001, time.Now(), 3*time.Second); len(around) != 1 || around[0].UserMsgID != 5 {
